...
if(((isset($_SESSION['isLoged']))&&(!empty($_SESSION['isLoged']))) && ((isset($_SESSION['id']))&&(!empty($_SESSION['id'])))  
				...
				/* Connect to database */
					...
					if($getType == "staff"){
						 $eQuery = "UPDATE  `doctor` SET  `released` =  'true' WHERE  `id` =$getId  LIMIT 1";
						 $e_solution = $draco5->update_db($eQuery);
							if($e_solution=="SUCCESS"){
							header("Location: deldoctor.php");
							exit();
							}else{  die ( 'Error, please contact admin. Thank You. ');}
					}else{
						 $eQuery = "UPDATE  `users` SET  `released` =  'true' WHERE  `id` =$getId  LIMIT 1";
						 $e_solution = $draco5->update_db($eQuery);
							if($e_solution=="SUCCESS"){
							header("Location: deldoctor.php");
							exit();
